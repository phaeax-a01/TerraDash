...
`source`, `tolerance`, and authored Admin-0 `locationIds`. Source resolution
controls available coastline detail, while tolerance controls projected path
simplification, so they remain separate fields. The generator resolves each
location by stable source key, requires exactly one match in both pinned
Admin-0 files, and packages only those referenced features in
`data/generated/context.json`.

The configured variant is used only for those retained context IDs; all other
context IDs fall back exactly to existing 50m map paths. Defaults and every
unconfigured quiz remain byte-stable. Preserve context membership and
composition: this option changes geometry fidelity, not selectable identities,
joins, hit areas, or exclusions.
...
scale. Validate deterministic source-key mapping, configured alternate-path
selection, exact fallback, all mapped-quiz regressions, and disposable
wide/mobile evidence for regional coastlines and representative quizzes.
